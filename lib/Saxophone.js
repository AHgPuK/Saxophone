    ...
    _parseChunk(input, callback) {
        ...
        while (chunkPos < end) {
            ...
            if (nextChar === '!') {
                ...
                if (nextNextChar == 'D' && input.slice(chunkPos, chunkPos + 7) == 'DOCTYPE')
                {
                    const doctypeClose = input.indexOf('>', chunkPos);
                    ...
                    this.emit(
                        Node.doctype,
                        {contents: input.slice(chunkPos + 8, doctypeClose)}
                    );

                    chunkPos = doctypeClose + 1;
                    continue;
                }
